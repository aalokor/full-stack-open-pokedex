...
Ruby aims to be simple and easy to set up, and the teams using it would most likely prefer to outsource their CI infrastructure. Therefore, a cloud-based environment would be a good choice. However, if the team is working in an enterprise context, a self-hosted environment could be a good option for more extensive control. For a smaller startup, a cloud CI system, such as Github Actions, works well.
